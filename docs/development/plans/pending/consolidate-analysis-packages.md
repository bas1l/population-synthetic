...
- [ ] `src/population_synth/analysis/` contains exactly four subpackages
...
- [ ] `python -m pytest tests/` passes unchanged, including the aggregator
      golden snapshot (`tests/data/expected_metrics.json` **not** edited).
- [ ] `ruff check src/ scripts/` is clean (no import left unused by the move).
- [ ] The import smoke test and all entry-point `--help` invocations resolve
      (see Testing Plan).
- [ ] `comparison → mapping` remains the only cross-process dependency direction
...
- [ ] 1.1 — Create the new tree with docstring-only `__init__.py` files:
...
- [ ] 1.2 — `git mv` the modules (preserve history):
      ...
      - Delete the now-empty old `comparison/__init__.py` and the empty
        `comparison/` dir.
- [ ] 1.3 — Rewrite intra-package absolute imports:
...
- [ ] 1.4 — Rewrite script imports.
- [ ] 1.5 — Rewrite test imports.
...
- [ ] 2.1 — `python -m pytest tests/` green (golden snapshot unmodified).
- [ ] 2.2 — `ruff check src/ scripts/` clean; drop any now-unused import.
- [ ] 2.3 — Import smoke test + entry-point `--help` all resolve.
- [ ] 2.4 — Update living docs (leave completed plans untouched):
...
### Unit Tests
- [ ] All 14 affected test files resolve against the new import paths and pass.
- [ ] Aggregator golden snapshot passes with `tests/data/expected_metrics.json`
      unchanged (a diff means an import rewrite altered behavior — rework, never
      edit the snapshot).

### Integration Tests
- [ ] Full `python -m pytest tests/` green after Phase 1.

### Manual Verification
- [ ] Import smoke test (editable install picks up new subpackages, no reinstall):
      ```bash
      ...
      ```
- [ ] `python scripts/analyze/map_populations.py --help`,
      `compare_pipeline_to_scb.py --help`, `analyze_run.py --help`,
      `compare_runs.py --help` all resolve imports.

### Edge Cases
- [ ] `__file__`-relative path math: confirm moved modules derive paths from
...
- [ ] Update `CLAUDE.md` Architecture section + old package-path references.
- [ ] Update `docs/architecture/sub-packages.md` (new `analysis/` layout).
- [ ] Update `docs/architecture/comparison-mapping.md` (module paths).
- [ ] Docstring-only `__init__.py` for `analysis/`, `analysis/mapping/`,
      `analysis/comparison/`, `analysis/utils/` naming each process.
- [ ] No user-facing README/command change (CLI + outputs unchanged).
