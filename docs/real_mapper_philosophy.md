# The Real-Population Mapper — Philosophy

This note explains *why* `analysis/mapping/real_mapper/` exists as its own thing,
what it is responsible for, and the principle that governs it: **the real
population is the source of truth for the comparison's category space.** It is the
companion of `analysis/mapping/synthetic_mapper/`, and the contrast between the two is
...
- the **real** population — real national-statistics data (SCB for Sweden,
...
| | real_mapper | synthetic_mapper (pipeline) |
|---|---|---|
...
| Rules block consulted | each attribute file's `real` block | each attribute file's `synthetic` block |
...
`analysis/mapping/mapping_engine.py` (`resolve`). Each per-country subclass
(`SwedishRealMapper`/`ItalianRealMapper`,
...
real side is thin *because the statistics agency already did the hard
...
clean coded-real path, even though they share the resolver.

## What the mapping actually does — relabel *and* collapse

It is tempting to read the real mapper as cosmetic ("make labels prettier").
...
## The governing principle: the real population defines the category space

The comparison must measure the synthetic data against *what the real
population actually contains* — nothing invented, nothing assumed. Concretely:

- **No empty canonical buckets.** A category only belongs in the comparison if the
  real population emits it. If SCB's labour-force extract only yields
...
- **No properties the real population lacks.** ISTAT provides no income-source field, so
...
response, every comparison category must come from real data.

> One subtlety: when a raw value matches no matcher, the real side resolves it
> to `None` (dropped from the comparison) rather than erroring or passing it through.
> So if the real population ever emits a label outside the curated `real` matchers, that
> mass silently disappears from the axis. A golden diff of the mapper over the real
> population file is how this is caught — every value the real data emits must be claimed by
> some matcher.
...
had a broad `output_categories`; the scheme narrowed it back to the real-grounded
...
- `values` — the unified category set **and** the chart/axis order. This is the
  real-grounded scored axis for the attribute;
- `real` — unified value → matcher, resolving a raw national-statistics value;
...
buckets" and "no real-absent properties", because the axis is exactly the values both
...
real population file changes and a new coded label appears, add it to the
matching value's `real` matcher; a label the real data emits but no matcher catches
resolves to `None` (dropped from the comparison), so a golden diff over the real
population file is the check that the config is complete.

## Age: a derived dimension, not a stored field

`age_group` is the one category the mappers used to *synthesize* — by binning the
raw integer `age`, which neither population stores as a category. Under the
real-grounded principle a synthesized category does not belong in the schema, so the
...
not pretend the real population provided them.
...
`load_real_population → map_population` vs
`load_synthetic_population → map_population` — so the symmetry is visible at every call
site while the clean and messy paths stay isolated. The real mapper is small
on purpose: that smallness is the evidence that the real population, not the code, is
