...
- **Comparison scripts:** `scripts/compare_pipeline_to_scb.py` (single run, Sweden),
  `scripts/compare_pipeline_to_istat.py` (Italy), `scripts/compare_all_pipelines.py` (batch).
...
Note: `normalizer.py::normalize_raw_to_schema` (the *reference* path) passes unmapped values
through via `_ci_get`; reported unmapped-% is therefore a **lower bound** where it's involved.
...
2. **No-regression on a clean run** (catches over-broad stems): regenerate a model that was
   ...
   ```
   python scripts/compare_pipeline_to_scb.py --model-id claude_haiku --strategy-id all_pick \
       --country-id <country> --no-charts --output <scratch>/check.json
   ```
   (Italy: `scripts/compare_pipeline_to_istat.py`.)
...
```
python scripts/compare_all_pipelines.py --country <country>     # all reports + charts
...
| Single-run compare | `scripts/compare_pipeline_to_scb.py` | `scripts/compare_pipeline_to_istat.py` |
