...
| File | What it covers |
|------|----------------|
| [`sub-packages.md`](sub-packages.md) | Full per-package breakdown of `src/population_synthetic/` (`generators/real`, `generators/synthetic`, `fidelity`, `gui`, `run_analytics`, `utils`, `clients`) plus `_paths.py` path resolution. |
