# Real-population generation diagrams

Per-country workflow / conditional-sampling DAGs for the three **real
...
treats as the ground truth. (For the LLM persona-generation side, see
...
Source modules: `src/population_synthetic/generators/real/{sweden,norway,italy}/{constants,fetch_service,parsers,sample_service}.py`
and the shared layer `generators/real/{data,helpers,income_class}.py`.
