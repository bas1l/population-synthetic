...
- `reference_mapper/` maps the **reference** (database) population -- raw national-statistics records (nested `RawCategory` dicts from SCB/ISTAT) -> canonical schema labels -- and is the structural counterpart to `synthetic_mapper/`: a class hierarchy `AbstractReferenceMapper -> BaseReferenceMapper -> {SwedishReferenceMapper, ItalianReferenceMapper}` with a `get_reference_mapper(country, mappings_path=None)` factory. Because the reference input is already cleanly coded, the attribute logic is a single mappings-driven loop that lives entirely in `BaseReferenceMapper` (no per-country method overrides). `BaseReferenceMapper` is a **generic handler-kind engine** that is **field-agnostic** by *config discovery* rather than a hardcoded column: each per-attribute mapping JSON self-declares its algorithm via a `reference_handler` key (one of `passthrough`/`label`/`composite`/`decile_coded`/`substring_coded`) plus an optional `reference_attr` (the output schema key when it differs from the file stem). `__init__` scans the loaded mappings and registers one generic handler per block that declares a `reference_handler` (blocks without the key -- `_scheme` and the synthetic-only files -- are skipped), so the class holds **zero field-name literals** and adding/removing a comparison field is a config-only change; it fails fast (`ValueError`) on an unknown handler kind or when no block declares one. The handler kind names a generic algorithm, never a field. It stays **label-agnostic** (it holds no canonical category string -- every output label comes from the country's mapping tables, read through role-based sub-keys: `reference_label_mappings`, `reference_attachment_mappings`/`reference_hours_mappings` + `reference_composite_mappings` for employment_type, `reference_decile_mappings`/`mappings[*].reference_codes` for the coded attributes, and `reference_none_default` for the absent-field fallback). Country divergence is reduced to a single subclass class attribute, `MAPPINGS_SUBDIR` (the mapping directory); even the domestic-birth collapse is an ordinary table entry, so the old `DOMESTIC_NAME`/`DOMESTIC_BIRTH_LABELS` attributes and the `if country == "italian"` branch are gone. The two-step API mirrors the synthetic side: `load_reference_population(path)` (reads the population *as it is on disk*) then `normalize_population(raw_pop, country)` (normalizes only if raw-format; an already-flat population passes through unchanged). Supporting primitives `load_mappings` / `is_raw_format` live in the package; `normalizer.py` is now a thin backward-compat facade (`normalize_raw_to_schema()` / `normalize_if_raw()` / `load_mappings` / `is_raw_format`) delegating to it, kept because `extract/mappings.py` imports `load_mappings` from there
- `synthetic_mapper/` maps the **synthetic** (pipeline) population -- raw `identity.json` free-text values -> canonical schema labels -- via a class hierarchy `AbstractSyntheticMapper -> BaseSyntheticMapper -> {SwedishSyntheticMapper, ItalianSyntheticMapper}`, so each country owns its specificities (no `is_italian` branching). `BaseSyntheticMapper` holds the orchestrator plus the cross-country attributes (age, ethnicity, environment, socioeconomic, parental_structure); subclasses implement the divergent ones. Narrative/batch parsing is a mapper method (Swedish only; Italian raises). The two-step API is `load_raw_population(seed_root)` (reads the population *as it is on disk*) then `map_population(raw_pop, country)` -- the compare scripts call both explicitly so load and map are visible, mirroring the reference side's `load_reference_population` -> `normalize_population`. `get_synthetic_mapper(country)` is the factory. The low-level translation primitives live in `extract/` (`normalizers_se`, `normalizers_it`, `schema_labels`, `mappings`, `batch`)
...
- **Simulation configs:** `config/synthetic/simulation_configs/` -- per-attribute `categories` schema + locale `instruction` system prompt for configurable mode. Strategy definitions are single-file yamls under `config/synthetic/axes/strategies/` (see Axis Composition System above). (Batch mode bundles no prompt files; a future all-properties-at-once batch generator is planned -- see `docs/development/plans/pending/batch-generate-all-properties.md`)
