"""
compare_model_performance.py -- Cross-model performance comparison (models x methods per country).

Sits AFTER the compare stage: consumes the per-combo comparison reports written by
compare_all_pipelines.py / compare_pipeline_to_scb.py under
{output_base}/03_Analysis/comparison/{slug}/{slug}.json and compares the model x strategy
...
compare_all_pipelines.py first.

Outputs, per country, under {output_base}/03_Analysis/performance/:
...
Usage:
    python scripts/analyze/compare_model_performance.py
    python scripts/analyze/compare_model_performance.py --country swedish
    python scripts/analyze/compare_model_performance.py --model claude_haiku --model gemini_flash
    python scripts/analyze/compare_model_performance.py --strategy all_pick --no-charts
    python scripts/analyze/compare_model_performance.py --slug swedish_all_pick_claude_haiku \
        --slug swedish_all_pick_gemini_flash
    python scripts/analyze/compare_model_performance.py --per-attribute-charts --strict
...
def main() -> None:
    ...
    performance_dir = output_base / "03_Analysis" / "performance"
    ...
    if not by_country:
        print(
            ...
            "Run scripts/analyze/compare_all_pipelines.py first.",
            file=sys.stderr,
        )
        sys.exit(1)
