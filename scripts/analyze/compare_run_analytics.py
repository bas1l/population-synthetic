"""
compare_run_analytics.py -- Cross-run scientific comparison of run analytics.

Reads the per-run ``run_analytics.json`` files written by ``analyze_run.py`` under
the ``run_analytics`` master folder, groups metrics by **model** and by
...
Usage:
    ...
    python scripts/analyze/compare_run_analytics.py --root path/to/run_analytics --country swedish
    python scripts/analyze/compare_run_analytics.py --metrics retry_rate wall_clock --output cmp.json
    python scripts/analyze/compare_run_analytics.py \
        --slug swedish_all_pick_claude_haiku --slug swedish_all_pick_gemini_flash
"""
...
from population_synthetic.analysis.run_analytics.cross_run.comparison_charts import plot_run_comparison
from population_synthetic.analysis.run_analytics.cross_run.run_comparison import (
    METRIC_SPECS,
    METRIC_SPECS_BY_KEY,
    build_comparison,
    load_run_records,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "methods. Reads per-run run_analytics.json files under the run_analytics "
            "master folder."
        )
    )
    parser.add_argument(
        "--root", type=Path, default=None, metavar="DIR",
        help="run_analytics master folder to scan (default: derived from config).",
    )
    parser.add_argument(
    ...
    if root is None:
        ...
        sys.exit(1)
    if not root.is_dir():
        print(f"Error: run_analytics root not found: {root}\n"
