...
The old dual-vocabulary drift guard (synthetic emittable vocab subseteq reference
...
invariant on disk -- i.e. neither the ``database`` nor the ``synthetic`` rules block
...
2. every non-directive key of the ``database`` block is a declared ``value``;
...
from population_synth.analysis.mapping.reference_mapper.mappings import load_index
...
def test_rules_blocks_key_only_declared_values(country: str, subdir: str) -> None:
    ...
    for attr, path, block, all_attrs in _country_attribute_files(subdir):
        ...
        for side in ("database", "synthetic"):
