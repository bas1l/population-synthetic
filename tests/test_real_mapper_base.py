"""Unit tests for ``BaseRealMapper`` as a thin loader over the shared engine.

The real mapper no longer holds a handler-kind engine: it reads the per-country
``_index.json`` master plus each per-attribute file's ``real`` block and ``values``
...
:mod:`tests._mapping_fixtures` (the new ``values`` / ``real`` / ``synthetic`` shape,
...
def _real_record() -> dict:
    """A raw real record (nested ``RawCategory`` dicts) exercising every branch."""
    return {
        "id": "real-1",
        "age": 30,
...
def test_missing_index_raises():
    ...
    with pytest.raises(ValueError, match="_index"):
        BaseRealMapper({"biological_sex": {"values": ["Male", "Female"]}})


def test_index_without_attributes_raises():
    with pytest.raises(ValueError, match="_index"):
        BaseRealMapper({"_index": {"description": "no attributes"}})
...
def test_missing_config_block_for_attribute_raises():
    ...
    mapper = BaseRealMapper(mappings)
...
def test_real_mapper_delegates_end_to_end():
    ...
    assert out["id"] == "real-1"                       # id passthrough
...
def test_real_absent_field_uses_absent_directive():
