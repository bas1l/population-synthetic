"""Synthetic-population mapper: raw pipeline identities -> canonical schema.

Public API:
- ``load_raw_population(seed_root)`` / ``map_population(raw_pop, country)`` -- the
  two-step load-then-map flow used by the compare scripts.
- ``get_synthetic_mapper(country)`` -- factory for the country mapper class.
- ``AbstractSyntheticMapper`` / ``BaseSyntheticMapper`` /
  ``SwedishSyntheticMapper`` / ``ItalianSyntheticMapper`` -- the class hierarchy.
"""

from population_synth.analysis.mapping.synthetic_mapper.base import (
    AbstractSyntheticMapper,
    BaseSyntheticMapper,
)
from population_synth.analysis.mapping.synthetic_mapper.factory import get_synthetic_mapper
from population_synth.analysis.mapping.synthetic_mapper.italy import ItalianSyntheticMapper
from population_synth.analysis.mapping.synthetic_mapper.loader import load_raw_population, map_population
from population_synth.analysis.mapping.synthetic_mapper.sweden import SwedishSyntheticMapper

__all__ = [
    "AbstractSyntheticMapper",
    "BaseSyntheticMapper",
    "SwedishSyntheticMapper",
    "ItalianSyntheticMapper",
    "get_synthetic_mapper",
    "load_raw_population",
    "map_population",
]
