...
``BaseReferenceMapper`` is a **thin loader** over the shared symmetric resolver
(:mod:`population_synth.analysis.mapping.mapping_engine`). It reads the per-country
...
:func:`mapping_engine.resolve`, passing that file's ``database`` rules block and
...
expressed as matchers in the ``database`` block and evaluated by the shared engine.
...
from population_synth.analysis.mapping import mapping_engine
...
class BaseReferenceMapper(AbstractReferenceMapper):
    ...
    each attribute's ``database`` rules block and ``values`` list drive
    ...
    def __init__(self, mappings: dict) -> None:
        ...
        *mappings* is the per-country config merged by ``load_mappings`` (one entry
        ...
        if not isinstance(index, dict) or not isinstance(index.get("attributes"), dict):
            raise ValueError("reference mapper requires an '_index' block declaring 'attributes'")
    ...
    def _block_for(self, attr: str) -> dict:
        ...
        if not isinstance(block, dict):
            raise ValueError(f"reference mapper: missing config block {stem!r} for attribute {attr!r}")
        return block
    ...
    def _resolve_attr(self, attr: str, record: dict, resolved: dict[str, Any]) -> Any:
        ...
        ``database`` block, resolving a ``refine_from`` sibling first when declared.
        """
        if attr in resolved:
