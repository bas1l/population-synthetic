...
Owns the data contract of the model-ranking process: the per-combo
    ...
    *attributes* is the country's config-sourced comparison axis; a report whose
    ...
    if missing or extra:
        raise KeyError(
            ...
            "The report is stale -- re-run scripts/analyze/score_fidelity_all.py."
        )
    ...
    *attributes_for_country* and *axis_ids* (``(country_ids, strategy_ids,
    ...
    if not index_path.is_file():
        raise FileNotFoundError(
            ...
            "and scripts/analyze/score_fidelity_all.py first."
        )
        ...
        if not report_path.is_file():
            if strict:
                raise RuntimeError(
                    ...
                    "Run scripts/analyze/score_fidelity_all.py first (or drop --strict)."
                )
