...
``per_category``
    Per-category call counts, retry rates, free-text error taxonomy, and the
    structured ``error_category`` breakdown.
...
def _per_category(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Two error breakdowns are kept side by side: ``error_taxonomy`` is the
    original free-text taxonomy (first "word" of the raw error string), and
    ``error_category_counts`` is the structured taxonomy (``network`` /
    ``timeout`` / ``auth`` / ``rate_limit`` / ``model_limitation`` /
    ``invalid_response`` / ``unknown``) carried by JSONL-native telemetry's
    ``error_category`` field (Phases 1-3).  The structured breakdown is
    ``None``-tolerant: entries without an ``error_category`` (legacy runs)
    simply don't contribute to it.
    """
    ...
    cat_error_categories: dict[str, dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    for e in entries:
        ...
        if error_category is not None:
            cat_error_categories[cat][error_category] += 1
    ...
    for cat in all_cats:
        ...
        per_category[cat] = {
            ...
            "error_category_counts": dict(cat_error_categories.get(cat, {})),
...
def compute_metrics(
    ...
    # Token-gating is source-agnostic: it fires whether prompt_tokens/
    # completion_tokens came from JSONL-native telemetry (Phases 1-3, present
    # directly on the entry -- e.g. a plain parse_interactions() call with no
    # log file) or from a log-join (joiner.join_entries), since both populate
    # the same entry dict keys.
    has_token_data = any(
