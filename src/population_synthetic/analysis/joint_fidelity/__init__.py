"""population_synthetic.analysis.joint_fidelity — standalone multivariate joint-fidelity.
...
analysis process with its own ``03_Analysis/joint_fidelity/`` output folder, separate
from the marginal comparison. Dataflow:

    build → write / aggregate → visualize

The metric itself is NOT re-implemented here: ``builder.build_joint_fidelity``
is a thin driver that instantiates the shared
:class:`~population_synthetic.analysis.comparison.evaluator.StatisticalEvaluator` over the
...
folders. The comparison evaluator, the performance loader/leaderboard, and the paper path
