"""builder.py -- Build and persist the standalone joint-fidelity envelope.

Wraps the shared multivariate computation
(:meth:`~population_synthetic.analysis.comparison.evaluator.StatisticalEvaluator.compute_multivariate`)
...
merely persists to its own ``03_Analysis/joint_fidelity/`` folder.

The built structures are directly JSON/CSV-serialisable; :func:`write_joint_fidelity_json`,
:func:`write_joint_fidelity_csv`, and (for the reused pairwise-association CSV)
:func:`~population_synthetic.analysis.comparison.evaluator.write_association_csv` persist them.
"""
...
def write_joint_fidelity_json(envelope: dict[str, Any], out_path: str | Path) -> Path:
    """Write one combo's joint-fidelity envelope to *out_path*."""
...
def aggregate_joint_fidelity(envelopes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Roll a country's joint-fidelity envelopes up into one summary row per combo.

    Each row carries the combo's identity (``slug``/``strategy``/``model``) and the four
    headline joint-fidelity scalars pulled straight from its multivariate block:
...
def write_joint_fidelity_csv(rows: list[dict[str, Any]], out_path: str | Path) -> Path:
    """Write the per-country joint-fidelity roll-up (one row per combo) to CSV."""
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
