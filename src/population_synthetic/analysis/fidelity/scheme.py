...
``config/analysis/fidelity/{country}.json``, which is pure evaluator tuning and has
...
_ANALYSIS_ROOT = PROJECT_ROOT / "config" / "analysis" / "fidelity"
...
def load_scheme(
    ...
    ``config/analysis/fidelity/{country}.json``.
