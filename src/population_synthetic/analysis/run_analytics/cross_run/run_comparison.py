...
:func:`population_synthetic.analysis.run_analytics.per_run.aggregator.compute_metrics` and persisted as
``run_analytics.json`` files under the ``run_analytics`` master folder) across the
two experimental factors: **model** and **method/strategy** (country held fixed).

Pipeline:
    1. :func:`load_run_records` (in :mod:`population_synthetic.analysis.run_analytics.cross_run.comparison_loader`)
...
:mod:`population_synthetic.analysis.run_analytics.cross_run.comparison_loader`.
"""
...
from population_synthetic.analysis.run_analytics.cross_run.comparison_loader import (
    METRIC_SPECS,
    METRIC_SPECS_BY_KEY,
    MetricSpec,
    RunRecord,
