...
DTO, the reshaping of a run's analytics dict into per-metric sample lists, and
the filesystem walk that loads all decodable ``run_analytics.json`` files under
the ``run_analytics`` master folder.  Slug decomposition lives in the shared
:mod:`population_synthetic.analysis.utils.axes`.
...
def load_run_records(
    ...
    *,
    ...
    """
    root = Path(run_analytics_root)
    if not root.is_dir():
        raise FileNotFoundError(f"run_analytics root not found: {root}")
